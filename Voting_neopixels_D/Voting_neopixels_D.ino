...
void loop(){
  ...
  if (millis() >= updateTime) {
    ...
    if (string != "" && string != prevString){
      ...
      if (((float(D)/float(totalTally)) * 100) > 74)
        dimmer = 1;
      else if (((float(D)/float(totalTally)) * 100) < 75 && ((float(D)/float(totalTally)) * 100) > 49)
        dimmer = 2;
      else if (((float(D)/float(totalTally)) * 100) < 50 && ((float(D)/float(totalTally)) * 100) > 24)
        dimmer = 3;
      else if (((float(D)/float(totalTally)) * 100) < 25 && ((float(D)/float(totalTally)) * 100) >= 0)
        dimmer = 4;
...
void setColor(){
  if (C == D){     // if C is equal to D
    ...
    red = 0;
    blue = map(D, 0, totalTally/2, 255, 0);
    green = map(D, 0, totalTally/2, 0, 255);
  }
  else if (C < D){   // C is less than D
    blue = 0;
    red = map(D, totalTally/2, totalTally, 0, 255);
    green = map(D, totalTally/2, totalTally, 255, 0);
  }
}
