...
// set flag to c(E,F)
GETrequest setFlag(ip, 80, "www.kisese.byethost7.com", "/kura/flags_process.php?flag=c");
...
void loop(){
  ...
  if (millis() >= updateTime) {
    ...
    if (string != "" && string != prevString){
      ...
      if (((float(E)/float(totalTally)) * 100) > 74)
        dimmer = 1;
      else if (((float(E)/float(totalTally)) * 100) < 75 && ((float(E)/float(totalTally)) * 100) > 49)
        dimmer = 2;
      else if (((float(E)/float(totalTally)) * 100) < 50 && ((float(E)/float(totalTally)) * 100) > 24)
        dimmer = 3;
      else if (((float(E)/float(totalTally)) * 100) < 25 && ((float(E)/float(totalTally)) * 100) >= 0)
        dimmer = 4;
